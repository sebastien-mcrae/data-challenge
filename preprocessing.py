...
class Preprocessing():
    def __init__(self):
        ...
        self.regex_patterns = re.compile(
            '(' + self.regex1 + '|' + self.regex2 + '|' + self.regex3 + '|' + self.regex4 + '|' + self.regex5 + '|' + self.regex6 + '|' + self.regex7 + ')',
            re.IGNORECASE)
    ...
    def date_processing(self, match):
        # Pattern 1
        if re.search(self.pattern1, match) is not None:
            tokens_date = self.word_tok(match)
            if len(tokens_date[0]) == 1:
                tokens_date[0] = '0' + tokens_date[0]
            date = tokens_date[2] + '-' + self.dict_month[tokens_date[1]] + '-' + tokens_date[0]

        # Pattern 2
        elif re.search(self.pattern2, match) is not None:
            date = match[6:10] + '-' + match[3:5] + '-' + match[0:2]

        # Pattern 3
        elif re.search(self.pattern3, match) is not None:
            if int(match[6:8]) <= 20:
                start_year = '20'
            else:
                start_year = '19'
        ...
        # Pattern 4
        elif re.search(self.pattern4, match) is not None:
            tokens_date = self.word_tok(match)
            date = tokens_date[2] + '-' + self.dict_month[tokens_date[1]] + '-' + '01'

        # Pattern 5
        elif re.search(self.pattern5, match) is not None:
            tokens_date = self.word_tok(match)
            date = tokens_date[2] + '-' + self.dict_month[tokens_date[1]] + '-' + '01'

        # Pattern 6
        elif re.search(self.pattern6, match) is not None:
            date = match[10:14] + '-' + match[5:7] + '-' + match[0:2]

        # Pattern 7
        elif re.search(self.pattern7, match) is not None:
